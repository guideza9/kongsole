module Kong
  ...
  class EntitySync
    ...
    # it's attached to, and needs those already synced.
    TYPES_IN_SYNC_ORDER = %w[service consumer route keyauth_credential basicauth_credential plugin].freeze
    ...
    # Syncs every M0-M3 entity type for a connection in dependency order, for
    ...
    def self.sync_one(connection:, client:, entity_type:, kong_id:)
      response = client.get("#{Kong::EntityTypes.fetch(entity_type).list_path}/#{kong_id}")
      body = response.body
    ...
    def fetch_page(offset)
      ...
      response = @client.get(@definition.list_path, params: params)
      body = response.body

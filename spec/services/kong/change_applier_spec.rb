...
RSpec.describe Kong::ChangeApplier do
  ...
  describe "#call on a PR-mode plan" do
    ...
    it "refuses a PR-mode plan from before changesets too" do
      plan = create(:change_plan, apply_mode: "pr", status: "pending")
      expect { Kong::ChangeApplier.new(change_plan: plan, client: nil, actor_username: "a").call }
        .to raise_error(Kong::ChangeGuardrails::Violation, /changeset/)
    end
  end
  ...
  describe "an env whose apply_mode changed after the plan was proposed (R1.3)" do
    ...
    # R8.7: a PR plan is refused outright now -- it only ever leaves through a changeset.
    it "refuses a PR plan once its env has moved to direct" do
      ...
      expect { described_class.new(change_plan: plan.reload, client: nil, actor_username: "a").call }
        .to raise_error(Kong::ChangeGuardrails::Violation, /changeset/i)
    end
  end
end
